...
@section('title', app()->getLocale() == 'ar' ? 'تقويم المعلم' : 'Teacher Calendar')
...
<div class="container-fluid">
    ...
    <div class="row mb-4">
        <div class="col-md-12">
            <h2 class="mb-1">{{ app()->getLocale() == 'ar' ? 'تقويم الجلسات' : 'My Teaching Calendar' }}</h2>
            ...
            <div class="card shadow-sm">
                <div class="card-header bg-primary text-white">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <div class="btn-group">
                            <a href="{{ route('teacher.calendar', ['month' => $date->copy()->subMonth()->month, 'year' => $date->copy()->subMonth()->year]) }}" 
                               class="btn btn-sm btn-light">
                                <i class="bi bi-chevron-left"></i>
                            </a>
                            <a href="{{ route('teacher.calendar') }}" class="btn btn-sm btn-light">
                                {{ app()->getLocale() == 'ar' ? 'اليوم' : 'Today' }}
                            </a>
                            <a href="{{ route('teacher.calendar', ['month' => $date->copy()->addMonth()->month, 'year' => $date->copy()->addMonth()->year]) }}" 
                               class="btn btn-sm btn-light">
                                                    ...
                                                    @if($daySessions->count() > 0)
                                                        <div class="sessions-container mt-1">
                                                            @foreach($daySessions as $session)
                                                                <a href="{{ route('teacher.session.details', $session->id) }}" 
                                                                   ...
                                                                   title="{{ app()->getLocale() == 'ar' ? 'الطالب' : 'Student' }}: {{ $session->student->name ?? 'N/A' }}">
                                                                    <i class="bi bi-clock"></i> {{ \Carbon\Carbon::parse($session->start_time)->format('h:i A') }}
                                                                    <br>
                                                                    <small style="font-size: 0.65rem;">{{ Str::limit($session->student->name ?? 'N/A', 15) }}</small>
                                                                </a>
                                                            @endforeach
                                                        </div>
                                                    @endif
                ...
                <div class="card shadow-sm mb-4 border-info">
                    <div class="card-header bg-info text-white">
                        <h6 class="mb-0">
                            ...
                            {{ app()->getLocale() == 'ar' ? 'جلسات اليوم' : 'Today\'s Sessions' }}
                        </h6>
                    </div>
                    <div class="card-body p-0">
                                    ...
                                    <div class="d-flex justify-content-between align-items-start">
                                        <div class="flex-grow-1">
                                            ...
                                            <p class="mb-1 text-muted small">
                                                <i class="bi bi-person"></i> {{ $session->student->name ?? 'N/A' }}
                                            </p>
                                            <p class="mb-0 small">
