    ...
    return (
        <div className="space-y-6 animate-fade-in">
            <div className="flex justify-between items-center">
                <h2 className="text-2xl font-bold text-[var(--text-main)]">{t.courses}</h2>
            ...
            {courses.length === 0 ? (
                <div className="text-center py-16 bg-[var(--light-bg)] rounded-[var(--radius-md)] border border-dashed border-[var(--border)]">
                    <BookOpen className="mx-auto h-12 w-12 text-[var(--text-muted)] mb-4" />
                    <p className="text-[var(--text-muted)]">You haven't created any courses yet.</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {courses.map(course => {
                        ...
                        return (
                            <div key={course.id} className="bg-white rounded-[var(--radius-md)] border border-[var(--border)] shadow-[var(--shadow-sm)] overflow-hidden hover:shadow-[var(--shadow-md)] transition-shadow flex flex-col">
                                <div className="h-32 bg-[var(--light-bg)] flex items-center justify-center text-[var(--text-muted)] overflow-hidden relative">
                                    {course.cover_image ? (
                                    ...
                                    <div className="absolute top-2 right-2 flex gap-2">
                                        <button onClick={() => handleEdit(course)} className="p-2 bg-white/90 backdrop-blur-sm rounded-full text-secondary hover:bg-primary hover:text-white transition-all shadow-[var(--shadow-sm)]">
                                            <Edit size={16} />
                                        </button>
                                    </div>
                                </div>
                                <div className="p-5 flex-1 flex flex-col">
                                    <div className="flex justify-between items-start mb-2">
                                        <h3 className="font-bold text-lg text-[var(--text-main)] line-clamp-1">{course.name}</h3>
                                        <span className={`text-[10px] px-2 py-0.5 rounded font-bold uppercase ${course.status === 'published' ? 'bg-primary-pale text-primary' : 'bg-[var(--light-bg)] text-[var(--text-muted)]'}`}>
                                            {course.status}
                                        </span>
                                    </div>
                                    ...
                                    <p className="text-sm text-[var(--text-muted)] mb-4 line-clamp-2">{course.description}</p>
                                    ...
                                    <div className="mt-auto">
                                        ...
                                        <div className="flex items-center justify-between pt-4 border-t border-[var(--border)]">
                                            <span className="text-lg font-bold text-primary">{Number(course.price).toFixed(2)} <span className="text-xs font-normal text-[var(--text-muted)]">{t.sar}</span></span>
                                            ...
                                            <div className="flex items-center">
                                                {isConfirming ? (
                                                    <div className="flex gap-1 animate-fade-in items-center">
                                                        ...
                                                        <button onClick={() => handleDelete(course.id)} className="h-8 w-8 bg-red-500 text-white rounded-lg flex items-center justify-center hover:bg-red-600 shadow-[var(--shadow-sm)]"><Check size={14} /></button>
                                                        <button onClick={() => setConfirmDeleteId(null)} className="h-8 w-8 bg-[var(--light-bg)] text-[var(--text-muted)] rounded-lg flex items-center justify-center hover:bg-[var(--border)] transition-colors" title="No"><X size={14} /></button>
                                                    </div>
                                                ) : (
                                                    <button onClick={() => setConfirmDeleteId(course.id)} className="text-[var(--text-muted)] hover:text-red-500 transition-colors p-2 rounded-full hover:bg-red-50">
                                                        <Trash2 size={18} />
                                                    </button>
                                                )}
                                            </div>
                ...
                <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2 custom-scrollbar">
                    ...
                    {!editingId && (
                        <div className="mb-4 w-full">
                            ...
                            <div className="border border-[var(--border)] rounded-lg p-3 bg-[var(--light-bg)] flex items-center justify-between">
                                <input
                                    type="file"
                                    accept="image/*"
                                    onChange={(e) => setCoverImage(e.target.files?.[0] || null)}
                                    className="text-sm text-[var(--text-muted)] file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-primary/10 file:text-primary hover:file:bg-primary/20"
                                />
                                {coverImage && <span className="text-xs text-primary font-medium">{coverImage.name}</span>}
                            </div>
                        </div>
                    )}
                    ...
                    <div className="mb-4 w-full">
                        ...
                        <textarea
                            className="w-full rounded-lg border border-[var(--border)] p-3 h-24 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"
                    ...
                    {/* Schedule Builder */}
                    <div className="border border-[var(--border)] rounded-[var(--radius-md)] p-4 bg-[var(--light-bg)]">
                        <h4 className="font-bold text-sm text-navy mb-3">Course Schedule Slots</h4>
                        ...
                        <div className="space-y-2">
                            {slots.map((s, idx) => (
                                <div key={idx} className="flex justify-between items-center bg-white p-2 rounded border border-[var(--border)] text-sm">
                                    <span className="font-medium text-navy">{s.day} at {s.time}</span>
                            ...
                            {slots.length === 0 && <p className="text-xs text-[var(--text-muted)] italic">No slots added yet.</p>}
                        </div>
                    </div>

                    {uploadProgress !== null && (
                        <div className="w-full bg-[var(--light-bg)] rounded-full h-2 mb-4 overflow-hidden">
                            <div
                                className="bg-primary h-full transition-all duration-300"
                                style={{ width: `${uploadProgress}%` }}
                            ></div>
                            <p className="text-[10px] text-[var(--text-muted)] mt-1 text-center font-bold">Uploading: {uploadProgress}%</p>
                        </div>
                    )}
