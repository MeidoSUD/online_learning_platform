...
export const OverviewTab: React.FC<OverviewTabProps> = ({ user, onNavigate }) => {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="bg-gradient-to-r from-primary to-blue-600 rounded-[var(--radius-md)] p-6 md:p-10 text-white shadow-[var(--shadow-lg)] shadow-blue-200">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="flex gap-3">
              ...
              <Button 
                className="bg-white text-primary hover:bg-[var(--light-bg)] border-0"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, idx) => (
          <div 
            key={idx} 
            className={`bg-white p-6 rounded-[var(--radius-md)] border border-[var(--border)] shadow-[var(--shadow-sm)] hover:shadow-[var(--shadow-md)] transition-all ${stat.action ? 'cursor-pointer group' : ''}`}
            onClick={stat.action}
          >
            <div className="flex items-start justify-between">
              <div>
                <p className="text-[var(--text-muted)] text-sm font-medium mb-1 flex items-center gap-1">
                    {stat.label}
                ...
                <h3 className="text-2xl font-bold text-[var(--text-main)]">
                  {stat.value} <span className="text-sm font-normal text-[var(--text-muted)]">{stat.currency}</span>
                </h3>
              </div>
              <div className={`p-3 rounded-[var(--radius-md)] ${stat.color} text-white shadow-[var(--shadow-lg)] shadow-${stat.color.replace('bg-', '')}/30`}>
                <stat.icon size={20} />
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-[var(--radius-md)] border border-[var(--border)] shadow-[var(--shadow-sm)] p-6">
          <div className="flex justify-between items-center mb-6">
              <h3 className="text-lg font-bold text-[var(--text-main)]">{t.upcomingLessons}</h3>
              <button 
                onClick={() => onNavigate && onNavigate('schedule')}
          ...
          <div className="space-y-4">
             {upcomingSessions.length === 0 ? (
                 <div className="text-center py-10 bg-[var(--light-bg)] rounded-[var(--radius-md)] border border-dashed border-[var(--border)] text-[var(--text-muted)]">
                 ...
                 upcomingSessions.map((session) => (
                   <div key={session.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-[var(--radius-md)] bg-[var(--light-bg)] hover:bg-[var(--light-bg)] transition-colors gap-4">
                     <div className="flex items-center gap-4">
                       <div className="h-12 w-12 rounded-[var(--radius-md)] bg-white text-primary flex flex-col items-center justify-center font-bold shadow-[var(--shadow-sm)] border border-[var(--border)]">
        ...
        <div className="space-y-6">
            <div className="bg-white rounded-[var(--radius-md)] border border-[var(--border)] shadow-[var(--shadow-sm)] p-6">
                <h3 className="text-lg font-bold text-[var(--text-main)] mb-4">{language === 'ar' ? 'إجراءات سريعة' : 'Quick Actions'}</h3>
                <div className="space-y-3">
                    <button 
                        onClick={() => onNavigate && onNavigate('private-lessons')}
                        className="w-full flex items-center justify-between p-3 rounded-[var(--radius-md)] bg-[var(--light-bg)] hover:bg-primary/5 hover:text-primary transition-colors text-sm font-medium text-navy"
                    >
