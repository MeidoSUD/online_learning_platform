  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-slate-900">{language === 'ar' ? 'حجوزاتي' : 'My Bookings'}</h2>
        {onViewCalendar && (
      ...
      <div className="flex flex-col gap-4 bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
        <div className="relative">
          ...
          <input type="text"
            ...
            className={`w-full py-2 rounded-lg border border-slate-200 focus:outline-none focus:border-primary ${direction === 'rtl' ? 'pr-10 pl-4' : 'pl-10 pr-4'}`}
          />
        </div>
            ...
            return (
              ...
              >
                <div className="flex items-start gap-4">
                  <div className="h-12 w-12 rounded-full bg-slate-100 overflow-hidden shrink-0 flex items-center justify-center text-lg font-bold text-slate-500">
                    {teacherPhoto ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <h3 className="font-semibold text-slate-900 truncate">{subjectName}</h3>
                    ...
                    <div className="flex flex-wrap items-center gap-3 mt-3 text-xs text-slate-500">
                      ...
                      <span className="flex items-center gap-1 bg-slate-50 px-2 py-1 rounded font-medium text-slate-700">
      ...
      <Modal isOpen={showDetails} onClose={() => { setShowDetails(false); setBookingDetails(null); }}
        ...
        {loadingDetails ? (
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-3 pb-4 border-b">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">{language === 'ar' ? 'مرجع' : 'Ref'}: {bookingDetails.reference}</p>
              </div>
            ...
            <div className="bg-slate-50 rounded-lg p-4 space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                <span className="text-slate-500">{language === 'ar' ? 'الجلسات' : 'Sessions'}</span>
            ...
            <div className="bg-slate-50 rounded-lg p-4 space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-slate-500">{language === 'ar' ? 'سعر الجلسة' : 'Price/session'}</span>
            ...
            {bookingDetails.payment && (
              <div className="bg-slate-50 rounded-lg p-4 text-sm">
                <div className="flex justify-between">
                  <span className="text-slate-500">{language === 'ar' ? 'حالة الدفع' : 'Payment'}</span>
                  <span className="font-medium capitalize">{bookingDetails.payment.status}</span>
                </div>
                {bookingDetails.payment.paid_at && (
                  <div className="flex justify-between mt-1">
                    <span className="text-slate-500">{language === 'ar' ? 'تاريخ الدفع' : 'Paid at'}</span>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {bookingDetails.sessions.map((session: any) => (
                    <div key={session.id} className="flex items-center justify-between bg-slate-50 rounded-lg p-3 text-sm">
                      <div>
                        <span className="font-medium">{language === 'ar' ? `جلسة ${session.session_number}` : `Session ${session.session_number}`}</span>
                        <span className="text-slate-500 mx-2">|</span>
                        <span className="text-slate-500">{session.session_date}</span>
                      </div>
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                        session.status === 'completed' ? 'bg-green-100 text-green-700' :
                        session.status === 'live' ? 'bg-purple-100 text-purple-700' :
          ...
          <p className="text-slate-500 text-center py-4">{language === 'ar' ? 'لا توجد تفاصيل' : 'No details available'}</p>
        )}
      </Modal>
    </div>
  );
