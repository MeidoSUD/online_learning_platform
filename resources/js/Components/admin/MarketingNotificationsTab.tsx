...
import { CalendarClock, Loader2, RefreshCw, Search, Send, Smartphone, Bell, Users, Mail } from 'lucide-react';
import { adminService } from '../../Services/api';
import { useLanguage } from '../../Contexts/LanguageContext';
import { useToast } from '../../Contexts/ToastContext';
import { Modal } from '../ui/Modal';

type Channel = 'push' | 'sms' | 'email' | 'both' | 'all';
type TargetType = 'all' | 'teachers' | 'students' | 'single_user' | 'multi_teachers' | 'multi_students';
...
export const MarketingNotificationsTab: React.FC = () => {
  ...
  const [selectedGroup, setSelectedGroup] = useState<SearchUser[]>([]);
  ...
  const copy = language === 'ar' ? {
    ...
    channel: 'قناة الإرسال', target: 'الجمهور المستهدف', push: 'إشعار فوري', sms: 'رسالة نصية', email: 'بريد إلكتروني', both: 'إشعار + SMS', allChannels: 'جميع القنوات (إشعار + SMS + بريد)',
    all: 'جميع المستخدمين', teachers: 'المعلمون فقط', students: 'الطلاب فقط', single: 'مستخدم محدد', multiTeachers: 'اختيار معلمين محددين', multiStudents: 'اختيار طلاب محددين', selectAtLeastOne: 'اختر مستخدمًا واحدًا على الأقل', search: 'ابحث بالاسم أو البريد أو الجوال',
    ...
    channel: 'Delivery channel', target: 'Target audience', push: 'Push notification', sms: 'SMS', email: 'Email', both: 'Push + SMS', allChannels: 'All Channels (Push + SMS + Email)',
    all: 'All users', teachers: 'Teachers only', students: 'Students only', single: 'Specific user', multiTeachers: 'Choose specific teachers', multiStudents: 'Choose specific students', selectAtLeastOne: 'Select at least one user', search: 'Search by name, email, or phone',
  ...
  useEffect(() => {
    const isMulti = targetType === 'multi_teachers' || targetType === 'multi_students';
    if (targetType !== 'single_user' && !isMulti || search.trim().length < 2) { setUsers([]); return; }
    const role = targetType === 'multi_teachers' ? 3 : targetType === 'multi_students' ? 4 : undefined;
    const timeout = window.setTimeout(async () => {
      try { const response = await adminService.searchMarketingUsers(search.trim(), role); setUsers(response.data ?? []); }
      catch { setUsers([]); }
    }, 300);
  ...
  const prepareSubmission = async (event: FormEvent) => {
    ...
    if (isMulti && selectedGroup.length === 0) {
      showToast(copy.selectAtLeastOne, 'error'); return;
    }
    if (mode === 'schedule' && !scheduledAt) {
      showToast(copy.pickTime, 'error'); return;
    }
    try {
      const response = await adminService.getMarketingAudienceCount(targetType, selectedUser?.id, isMulti ? selectedGroup.map(u => u.id) : undefined);
  ...
  const submit = async () => {
    ...
    setSubmitting(true);
    try {
      await adminService.sendMarketingNotification({ title, body, channel, target_type: targetType, target_user_id: isMulti ? null : selectedUser?.id, target_user_ids: isMulti ? selectedGroup.map(u => u.id) : null, scheduled_at: willSchedule ? scheduledAt : null });
      showToast(willSchedule ? copy.scheduleOk : copy.sendOk, 'success');
      setTitle(''); setBody(''); setScheduledAt(''); setMode('now'); setSelectedUser(null); setSelectedGroup([]); setSearch(''); setConfirming(false); loadCampaigns();
    } catch (error: any) {
      const details = error?.errors && typeof error.errors === 'object'
        ? Object.keys(error.errors)
            .map(k => `${k}: ${Array.isArray(error.errors[k]) ? error.errors[k].join(', ') : error.errors[k]}`)
  ...
  const targetLabel = (campaign: Campaign) =>
    ({ all: copy.all, teachers: copy.teachers, students: copy.students, multi_teachers: copy.multiTeachers, multi_students: copy.multiStudents, single_user: campaign.target_user ? `${campaign.target_user.first_name} ${campaign.target_user.last_name}` : copy.single }[campaign.target_type]);
  ...
  return <div className="space-y-6 animate-fade-in" dir={direction}>
    ...
    <form onSubmit={prepareSubmission} className="bg-white rounded-[var(--radius-md)] border border-[var(--border)] p-5 sm:p-6 space-y-5">
      ...
      <div className="grid md:grid-cols-2 gap-5"><fieldset><legend className="text-sm font-medium text-[var(--text-main)] mb-2">{copy.channel}</legend><div className="flex flex-wrap gap-2">{([['push', copy.push, Bell], ['sms', copy.sms, Smartphone], ['email', copy.email, Mail], ['both', copy.both, Send], ['all', copy.allChannels, Users]] as const).map(([value, label, Icon]) => <label key={value} className={`cursor-pointer rounded-lg border px-3 py-2 text-sm flex gap-2 items-center ${channel === value ? 'border-primary bg-primary-pale text-primary' : 'border-[var(--border)]'}`}><input className="sr-only" type="radio" checked={channel === value} onChange={() => setChannel(value)} /><Icon size={16}/>{label}</label>)}</div></fieldset>
        <label className="text-sm font-medium text-[var(--text-main)]">{copy.target}<select value={targetType} onChange={e => { setTargetType(e.target.value as TargetType); setSelectedUser(null); setSelectedGroup([]); setSearch(''); }} className="mt-2 w-full rounded-lg border border-[var(--border)] px-3 py-2.5 outline-none focus:border-primary"><option value="all">{copy.all}</option><option value="teachers">{copy.teachers}</option><option value="students">{copy.students}</option><option value="single_user">{copy.single}</option><option value="multi_teachers">{copy.multiTeachers}</option><option value="multi_students">{copy.multiStudents}</option></select></label></div>
      {(targetType === 'single_user' || isMulti) && <div className="relative max-w-xl"><label className="text-sm font-medium text-[var(--text-main)]">{copy.search}<div className="relative mt-2"><Search className="absolute top-3 left-3 text-[var(--text-muted)]" size={17}/><input value={isMulti ? search : (selectedUser ? selectedUser.name : search)} onChange={e => { setSelectedUser(null); setSearch(e.target.value); }} placeholder={isMulti ? copy.multiTeachers : copy.single} className="w-full rounded-lg border border-[var(--border)] py-2.5 pl-10 pr-3 outline-none focus:border-primary" /></div></label>{isMulti && selectedGroup.length > 0 && <div className="mt-2 flex flex-wrap gap-2">{(selectedGroup).map(user => <span key={user.id} className="inline-flex items-center gap-1 rounded-full bg-primary-pale px-3 py-1 text-xs text-primary"><span>{user.name}</span><button type="button" onClick={() => setSelectedGroup(selectedGroup.filter(u => u.id !== user.id))} className="text-primary/70 hover:text-primary" aria-label="remove">×</button></span>)}</div>}{users.length > 0 && (!selectedUser || isMulti) && <div className="absolute z-20 mt-1 w-full rounded-lg border border-[var(--border)] bg-white shadow-lg">{users.map(user => { const added = isMulti && selectedGroup.some(u => u.id === user.id); return <button type="button" key={user.id} onClick={() => { if (isMulti) { setSelectedGroup(added ? selectedGroup.filter(u => u.id !== user.id) : [...selectedGroup, user]); setSearch(''); } else { setSelectedUser(user); setUsers([]); } }} className="flex w-full items-center justify-between px-3 py-2 text-left hover:bg-[var(--light-bg)]"><span><span className="block font-medium">{user.name}</span><span className="text-xs text-[var(--text-muted)]">{user.email || user.phone_number}</span></span>{added && <span className="text-xs font-medium text-primary">✓</span>}</button>; })}</div>}</div>}
