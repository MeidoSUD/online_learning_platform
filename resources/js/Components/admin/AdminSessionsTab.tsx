...
export const AdminSessionsTab: React.FC = () => {
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            ...
            case 'scheduled': return <span className="px-2 py-1 bg-secondary-pale text-secondary text-xs font-bold rounded">{language === 'ar' ? 'مجدول' : 'Scheduled'}</span>;
            case 'ended': return <span className="px-2 py-1 bg-[var(--light-bg)] text-navy text-xs font-bold rounded">{language === 'ar' ? 'مكتمل' : 'Ended'}</span>;
            ...
            default: return <span className="px-2 py-1 bg-[var(--light-bg)] text-[var(--text-muted)] text-xs font-bold rounded">{status}</span>;
        }
    };
    ...
    return (
        <div className="space-y-6 animate-fade-in relative">
            ...
            {/* Filters Bar */}
            <div className="bg-white p-4 rounded-[var(--radius-md)] border border-[var(--border)] shadow-[var(--shadow-sm)] flex flex-wrap gap-4 items-end">
                <div className="flex-1 min-w-[150px]">
                    <label className="text-xs font-bold text-[var(--text-muted)] mb-1 block">{t.teacherName}</label>
                    <input type="text" className="w-full p-2 rounded-lg border border-[var(--border)] text-sm focus:outline-none focus:border-primary" placeholder="Search Teacher..." value={filterTeacher} onChange={(e) => setFilterTeacher(e.target.value)} />
                </div>
                <div className="flex-1 min-w-[150px]">
                    <label className="text-xs font-bold text-[var(--text-muted)] mb-1 block">{language === 'ar' ? 'اسم الطالب' : 'Student Name'}</label>
                    <input type="text" className="w-full p-2 rounded-lg border border-[var(--border)] text-sm focus:outline-none focus:border-primary" placeholder="Search Student..." value={filterStudent} onChange={(e) => setFilterStudent(e.target.value)} />
                </div>
                <div className="flex-1 min-w-[150px]">
                    <label className="text-xs font-bold text-[var(--text-muted)] mb-1 block">{t.date}</label>
                    <input type="date" className="w-full p-2 rounded-lg border border-[var(--border)] text-sm focus:outline-none focus:border-primary" value={filterDate} onChange={(e) => setFilterDate(e.target.value)} />
                </div>
                <div className="flex-1 min-w-[150px]">
                    <label className="text-xs font-bold text-[var(--text-muted)] mb-1 block">{t.status}</label>
                    <select className="w-full p-2 rounded-lg border border-[var(--border)] text-sm focus:outline-none focus:border-primary bg-white" value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)}>
                ...
                <button onClick={clearFilters} className="p-2 text-[var(--text-muted)] hover:text-red-500 transition-colors" title="Clear Filters"><X size={20} /></button>
            </div>
                        ...
                        <table className="w-full text-left text-sm whitespace-nowrap">
                            <thead className="bg-[var(--light-bg)] border-b border-[var(--border)]">
                                <tr>
                                    ...
                                    <th className="px-6 py-4 font-bold text-navy">{language === 'ar' ? 'الوقت' : 'Time'}</th>
                                    <th className="px-6 py-4 font-bold text-navy">{t.teacher}</th>
                                    <th className="px-6 py-4 font-bold text-navy">{language === 'ar' ? 'الطالب' : 'Student'}</th>
                                    <th className="px-6 py-4 font-bold text-navy print:hidden text-right">{language === 'ar' ? 'إجراءات' : 'Actions'}</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-[var(--border)]">
                                {paginatedSessions.map(session => (
                                    <tr key={session.id} className="hover:bg-[var(--light-bg)] transition-colors">
                                        <td className="px-6 py-4">{getStatusBadge(session.status)}</td>
                                        <td className="px-6 py-4 font-medium text-navy">{new Date(session.session_date).toLocaleDateString()}</td>
                                        <td className="px-6 py-4 text-[var(--text-muted)]">{session.start_time.substring(0, 5)} - {session.end_time.substring(0, 5)}</td>
                                        ...
                                        <td className="px-6 py-4 print:hidden text-right">
                                            <button 
                                                ...
                                                className="px-3 py-1.5 bg-secondary-pale text-secondary hover:bg-secondary-pale font-semibold text-xs rounded-lg transition-colors border border-secondary/30"
                                ...
                                {paginatedSessions.length === 0 && (
                                    <tr><td colSpan={6} className="p-8 text-center text-[var(--text-muted)]">{language === 'ar' ? 'لا توجد جلسات' : 'No sessions found'}</td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
            ...
            {makeupModal.isOpen && makeupModal.session && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-[var(--radius-md)] shadow-xl w-full max-w-md overflow-hidden animate-slide-up">
                        <div className="p-6 border-b border-[var(--border)] flex justify-between items-center bg-secondary-pale">
                            ...
                            <button onClick={() => setMakeupModal({ isOpen: false, session: null })} className="text-[var(--text-muted)] hover:text-navy">
                                <X size={20} />
                            </button>
                        </div>
                        <div className="p-6">
                            ...
                            <div className="bg-[var(--light-bg)] p-4 rounded-[var(--radius-md)] border border-[var(--border)] mb-6 space-y-2 text-sm">
                                <div className="flex justify-between">
                                    <span className="text-[var(--text-muted)]">{language === 'ar' ? 'التاريخ الحالي:' : 'Current Date:'}</span>
                                    <span className="font-medium text-[var(--text-main)] line-through">{makeupModal.session.session_date}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-[var(--text-muted)]">{language === 'ar' ? 'التاريخ الجديد:' : 'New Date:'}</span>
                                ...
                                <div className="flex justify-between">
                                    ...
                                    <span className="font-medium text-[var(--text-main)]">{makeupModal.session.start_time.substring(0, 5)} - {makeupModal.session.end_time.substring(0, 5)}</span>
                                </div>
                            </div>

                            <div className="flex gap-3">
                                <button onClick={() => setMakeupModal({ isOpen: false, session: null })} className="flex-1 py-2.5 px-4 rounded-[var(--radius-md)] font-semibold text-[var(--text-muted)] bg-[var(--light-bg)] hover:bg-[var(--border)] transition-colors">
                                    {language === 'ar' ? 'إلغاء' : 'Cancel'}
                                </button>
                                <button onClick={handleMakeupSubmit} className="flex-1 py-2.5 px-4 rounded-[var(--radius-md)] font-bold text-white bg-primary hover:bg-primary shadow-[var(--shadow-md)] transition-colors">
            ...
            {userProfileModal?.isOpen && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-[var(--radius-md)] shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden animate-slide-up">
                        <div className="p-6 border-b border-[var(--border)] flex justify-between items-center">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h3 className="font-bold text-xl text-[var(--text-main)]">{userProfileModal.name}</h3>
                                    <p className="text-sm font-medium text-[var(--text-muted)] uppercase tracking-wider">{userProfileModal.role} Profile</p>
                                </div>
                            </div>
                            <button onClick={() => setUserProfileModal(null)} className="p-2 text-[var(--text-muted)] hover:text-navy bg-[var(--light-bg)] rounded-full hover:bg-[var(--light-bg)] transition-colors">
                                <X size={20} />
                            </button>
                        </div>
                        
                        <div className="p-6 bg-[var(--light-bg)] border-b border-[var(--border)] flex gap-6 overflow-x-auto">
                            <div className="bg-white p-4 rounded-[var(--radius-md)] shadow-[var(--shadow-sm)] border border-[var(--border)] flex-1 min-w-[150px]">
                                <p className="text-sm text-[var(--text-muted)] font-medium mb-1">{language === 'ar' ? 'إجمالي الجلسات' : 'Total Sessions'}</p>
                                <p className="text-3xl font-bold text-[var(--text-main)]">{userSessions.length}</p>
                            </div>
                            <div className="bg-white p-4 rounded-[var(--radius-md)] shadow-[var(--shadow-sm)] border border-[var(--border)] flex-1 min-w-[150px]">
                                <p className="text-sm text-[var(--text-muted)] font-medium mb-1">{language === 'ar' ? 'مكتملة' : 'Completed'}</p>
                                <p className="text-3xl font-bold text-primary">{userSessions.filter(s => s.status === 'ended').length}</p>
                            </div>
                            <div className="bg-white p-4 rounded-[var(--radius-md)] shadow-[var(--shadow-sm)] border border-[var(--border)] flex-1 min-w-[150px]">
                                <p className="text-sm text-[var(--text-muted)] font-medium mb-1">{language === 'ar' ? 'مجدولة' : 'Scheduled'}</p>
                                <p className="text-3xl font-bold text-secondary">{userSessions.filter(s => s.status === 'scheduled').length}</p>
                            </div>
                        </div>

                        <div className="p-6 flex-1 overflow-y-auto bg-white">
                            <h4 className="font-bold text-[var(--text-main)] mb-4 flex items-center gap-2">
                                <CalendarIcon size={18} className="text-[var(--text-muted)]"/> {language === 'ar' ? 'سجل الجلسات' : 'Session History'}
                            </h4>
                            ...
                            {loadingUserSessions ? (
                                ...
                                <div className="space-y-3">
                                    {userSessions.map(session => (
                                        <div key={session.id} className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-4 rounded-[var(--radius-md)] border border-[var(--border)] hover:border-primary/30 hover:shadow-[var(--shadow-md)] transition-all bg-[var(--light-bg)]">
                                            <div className="flex items-center gap-4">
                                                <div className="w-2 h-12 rounded-full bg-[var(--border)]"></div>
                                                <div>
                                                    <p className="font-bold text-[var(--text-main)]">{new Date(session.session_date).toLocaleDateString()}</p>
                                                    <p className="text-sm font-medium text-[var(--text-muted)]">{session.start_time.substring(0,5)} - {session.end_time.substring(0,5)}</p>
                                                </div>
                                            </div>
                                            
                                            <div className="flex items-center gap-4 mt-3 sm:mt-0 w-full sm:w-auto">
                                                <div className="text-sm text-[var(--text-muted)] bg-white px-3 py-1.5 rounded-lg border border-[var(--border)] flex items-center gap-2">
                                ...
                                <div className="text-center p-12 bg-[var(--light-bg)] rounded-[var(--radius-md)] border border-dashed border-[var(--border)]">
                                    <Video size={32} className="mx-auto text-[var(--text-muted)] mb-3" />
                                    <p className="text-[var(--text-muted)] font-medium">{language === 'ar' ? 'لا توجد جلسات لهذا المستخدم.' : 'No sessions found for this user.'}</p>
                                </div>
                            )}
                        </div>
                    </div>
