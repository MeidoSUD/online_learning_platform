...
export const UserProfileModal: React.FC<UserProfileModalProps> = ({ isOpen, onClose, userId }) => {
    ...
    const getStatusBadge = (status: string) => {
        const colorMap: Record<string, string> = {
            ...
            confirmed: 'bg-secondary-pale text-secondary',
            ...
            scheduled: 'bg-secondary-pale text-secondary',
            live: 'bg-primary-pale text-primary',
            ended: 'bg-[var(--light-bg)] text-navy',
        };
        return (
            <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${colorMap[status] || 'bg-[var(--light-bg)] text-[var(--text-muted)]'}`}>
                {status}
            </span>
        );
    };
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-8 pb-8" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
            ...
            <div className="relative bg-white rounded-[var(--radius-md)] shadow-2xl w-[95%] max-w-6xl max-h-[90vh] flex flex-col overflow-hidden animate-scale-in">
                {/* Header */}
                <div className="flex items-center justify-between p-6 border-b border-[var(--border)] flex-shrink-0">
                    ...
                    <button onClick={onClose} className="p-2 rounded-lg hover:bg-[var(--light-bg)] text-[var(--text-muted)] hover:text-[var(--text-muted)]">
                        <X size={20} />
                    </button>
                </div>
                ...
                {loading ? (
                    ...
                    <div className="flex-1 flex items-center justify-center py-20">
                        <p className="text-[var(--text-muted)]">{language === 'ar' ? 'لا توجد بيانات' : 'No data found'}</p>
                    </div>
                ) : (
                    <>
                        {/* User Info Bar */}
                        <div className="px-6 py-4 bg-gradient-to-r from-primary/5 to-transparent border-b border-[var(--border)] flex-shrink-0">
                            <div className="flex items-center gap-4">
                                <div className="h-14 w-14 rounded-full bg-[var(--light-bg)] flex items-center justify-center text-2xl font-bold text-[var(--text-muted)] flex-shrink-0">
                                    {user.first_name?.charAt(0)}{user.last_name?.charAt(0)}
                                </div>
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2 flex-wrap">
                                        <h3 className="text-lg font-bold text-[var(--text-main)]">{user.first_name} {user.last_name}</h3>
                                        ...
                                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${user.is_active ? 'bg-primary-pale text-primary' : 'bg-red-100 text-red-700'}`}>
                                ...
                                <div className="text-xs text-[var(--text-muted)] text-right flex-shrink-0 hidden sm:block">
                        ...
                        {/* Tab Navigation */}
                        <div className="flex-shrink-0 overflow-x-auto border-b border-[var(--border)] bg-[var(--light-bg)]">
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`flex items-center gap-1.5 px-3.5 py-3 text-xs font-medium border-b-2 transition-all whitespace-nowrap ${isActive
                                                ...
                                                : 'border-transparent text-[var(--text-muted)] hover:text-navy hover:bg-white/50'
                                                }`}
                                        >
                                            <Icon size={15} />
                                            {tab.label}
                                            {tab.badge !== undefined && tab.badge > 0 && (
                                                <span className={`ml-1 px-1.5 py-0.5 rounded-full text-[9px] font-bold ${isActive ? 'bg-primary/10 text-primary' : 'bg-[var(--border)] text-[var(--text-muted)]'}`}>
                                                    {tab.badge}
                                                </span>
                                            )}
                                        </button>
                            ...
                            {activeTab === 'bookings' && (
                                <div className="space-y-4">
                                    {profile?.bookings_as_student?.length > 0 && (
                                        <div>
                                            <h4 className="text-sm font-bold text-[var(--text-muted)] mb-2">{language === 'ar' ? 'حجوزات كطالب' : 'Bookings as Student'}</h4>
                                            <div className="overflow-x-auto rounded-[var(--radius-md)] border border-[var(--border)]">
                                                <table className="w-full text-sm">
                                                    <thead className="bg-[var(--light-bg)] text-xs text-[var(--text-muted)] uppercase">
                                                        <tr>
                                                    ...
                                                    <tbody className="divide-y divide-[var(--border)]">
                                                        {(profile?.bookings_as_student || []).map((b: any) => (
                                                            <tr key={b.id} className="hover:bg-[var(--light-bg)]">
                                                                <td className="px-4 py-2.5 font-medium text-navy">{b.reference || `#${b.id}`}</td>
                                                                ...
                                                                <td className="px-4 py-2.5 text-[var(--text-muted)] text-xs">{formatDate(b.created_at)}</td>
                                                            </tr>
                                                        ))}
                                                    </tbody>
                                                </table>
                                    ...
                                    {profile?.bookings_as_teacher?.length > 0 && (
                                        <div>
                                            <h4 className="text-sm font-bold text-[var(--text-muted)] mb-2 mt-4">{language === 'ar' ? 'حجوزات كمعلم' : 'Bookings as Teacher'}</h4>
                                            <div className="overflow-x-auto rounded-[var(--radius-md)] border border-[var(--border)]">
                                                <table className="w-full text-sm">
                                                    <thead className="bg-[var(--light-bg)] text-xs text-[var(--text-muted)] uppercase">
                                                        <tr>
                                    ...
                                    {(!profile?.bookings_as_student?.length && !profile?.bookings_as_teacher?.length) && (
                                        <p className="text-center text-[var(--text-muted)] py-8">{language === 'ar' ? 'لا توجد حجوزات' : 'No bookings found'}</p>
                                    )}
                                </div>
                            )}
                            ...
                            {activeTab === 'sessions' && (
                                <div className="space-y-4">
                                    {profile?.sessions_as_student?.length > 0 && (
                                        <div>
                                            <h4 className="text-sm font-bold text-[var(--text-muted)] mb-2">{language === 'ar' ? 'جلسات كطالب' : 'Sessions as Student'}</h4>
                                            <div className="overflow-x-auto rounded-[var(--radius-md)] border border-[var(--border)]">
                                                <table className="w-full text-sm">
                                                    <thead className="bg-[var(--light-bg)] text-xs text-[var(--text-muted)] uppercase">
                                                        <tr>
                                                    ...
                                                    <tbody className="divide-y divide-[var(--border)]">
                                                        {(profile?.sessions_as_student || []).map((s: any) => (
                                                            <tr key={s.id} className="hover:bg-[var(--light-bg)]">
                                                                <td className="px-4 py-2.5 font-medium text-navy">#{s.id}</td>
                                                                ...
                                                                <td className="px-4 py-2.5 text-xs text-[var(--text-muted)]">{s.session_date ? formatDate(s.session_date) : '-'}</td>
                                    ...
                                    {profile?.sessions_as_teacher?.length > 0 && (
                                        <div>
                                            <h4 className="text-sm font-bold text-[var(--text-muted)] mb-2 mt-4">{language === 'ar' ? 'جلسات كمعلم' : 'Sessions as Teacher'}</h4>
                                            <div className="overflow-x-auto rounded-[var(--radius-md)] border border-[var(--border)]">
                                                <table className="w-full text-sm">
                                                    <thead className="bg-[var(--light-bg)] text-xs text-[var(--text-muted)] uppercase">
                                                        <tr>
                                    ...
                                    {(!profile?.sessions_as_student?.length && !profile?.sessions_as_teacher?.length) && (
                                        <p className="text-center text-[var(--text-muted)] py-8">{language === 'ar' ? 'لا توجد جلسات' : 'No sessions found'}</p>
                                    )}
                                </div>
                            )}
                            ...
                            {activeTab === 'payments' && (
                                <div>
                                    {(profile?.payments?.length || 0) > 0 ? (
                                            ...
                                            <table className="w-full text-sm">
                                                ...
                                                <tbody className="divide-y divide-[var(--border)]">
                                                    {(profile?.payments || []).map((p: any) => (
                                                        <tr key={p.id} className="hover:bg-[var(--light-bg)]">
                                                            <td className="px-4 py-2.5 font-mono text-xs text-[var(--text-muted)]">#{p.id}</td>
                                                            <td className="px-4 py-2.5 font-semibold text-navy">{p.amount} {p.currency || 'SAR'}</td>
                                                            <td className="px-4 py-2.5 text-xs">{p.payment_method || '-'}</td>
                                                            <td className="px-4 py-2.5">{getStatusBadge(p.status)}</td>
                                                            <td className="px-4 py-2.5 text-xs text-[var(--text-muted)]">{p.booking?.reference || '-'}</td>
                                                            <td className="px-4 py-2.5 text-xs text-[var(--text-muted)]">{formatDateTime(p.created_at)}</td>
                                                        </tr>
                                                    ))}
                                                </tbody>
                                            </table>
                                        ...
                                        <p className="text-center text-[var(--text-muted)] py-8">{language === 'ar' ? 'لا توجد مدفوعات' : 'No payments found'}</p>
                                    )}
                                </div>
                            )}
                            ...
                            {activeTab === 'subscriptions' && (
                                <div>
                                    {(profile?.subscriptions?.length || 0) > 0 ? (
                                                ...
                                                <tbody className="divide-y divide-[var(--border)]">
                                                    {(profile?.subscriptions || []).map((s: any) => (
                                                        <tr key={s.id} className="hover:bg-[var(--light-bg)]">
                                                            ...
                                                            <td className="px-4 py-2.5">{getStatusBadge(s.status)}</td>
                                                            <td className="px-4 py-2.5 text-xs text-[var(--text-muted)]">{formatDate(s.start_date || s.created_at)}</td>
                                                            <td className="px-4 py-2.5 text-xs text-[var(--text-muted)]">{formatDate(s.end_date) || '-'}</td>
                                                        </tr>
                                                    ))}
                                                </tbody>
                                            </table>
                                        ...
                                        <p className="text-center text-[var(--text-muted)] py-8">{language === 'ar' ? 'لا توجد اشتراكات' : 'No subscriptions found'}</p>
                                    )}
                                </div>
                            )}
                            ...
                            {activeTab === 'orders' && (
                                <div>
                                    {(profile?.orders?.length || 0) > 0 ? (
                                            ...
                                            <table className="w-full text-sm">
                                                ...
                                                <tbody className="divide-y divide-[var(--border)]">
                                                    {(profile?.orders || []).map((o: any) => (
                                                        <tr key={o.id} className="hover:bg-[var(--light-bg)]">
                                                            <td className="px-4 py-2.5 font-mono text-xs text-[var(--text-muted)]">#{o.id}</td>
                                                            ...
                                                            <td className="px-4 py-2.5 text-xs text-[var(--text-muted)]">{formatDate(o.created_at)}</td>
                                                        </tr>
                                                    ))}
                                                </tbody>
                                            </table>
                                        ...
                                        <p className="text-center text-[var(--text-muted)] py-8">{language === 'ar' ? 'لا توجد طلبات' : 'No orders found'}</p>
                                    )}
                                </div>
                            )}
                                    ...
                                    {(profile?.courses?.length || 0) > 0 ? (
                                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                                            {(profile?.courses || []).map((c: any) => (
                                                <div key={c.id} className="p-4 rounded-[var(--radius-md)] border border-[var(--border)] hover:border-primary/30 hover:shadow-[var(--shadow-sm)] transition-all">
                                                    <div className="font-semibold text-[var(--text-main)] text-sm">{c.name}</div>
                                                    <div className="text-xs text-[var(--text-muted)] mt-1">{c.category?.name_en || c.category?.name_ar || '-'}</div>
                                                    <div className="flex items-center justify-between mt-2">
                                                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${c.status === 'published' ? 'bg-primary-pale text-primary' : 'bg-amber-100 text-amber-700'}`}>
                                                            {c.status}
                                                        </span>
                                                        <span className="text-xs font-semibold text-navy">{c.price} {language === 'ar' ? 'ر.س' : 'SAR'}</span>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
                                    ) : (
                                        <p className="text-center text-[var(--text-muted)] py-8">{language === 'ar' ? 'لا توجد دورات' : 'No courses found'}</p>
                                    )}
                                </div>
                            )}
                            ...
                            {activeTab === 'wallet' && (
                                <div className="space-y-6">
                                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                        <div className="p-4 rounded-[var(--radius-md)] bg-gradient-to-br from-primary/10 to-primary/5 border border-primary/20">
                                            <div className="text-xs text-[var(--text-muted)] mb-1">{language === 'ar' ? 'الرصيد الحالي' : 'Current Balance'}</div>
                                            <div className="text-2xl font-bold text-primary">
                                                {profile?.wallet?.balance || 0} {language === 'ar' ? 'ر.س' : 'SAR'}
                                            </div>
                                        </div>
                                        <div className="p-4 rounded-[var(--radius-md)] bg-gradient-to-br from-amber-50 to-amber-50/50 border border-amber-200">
                                            <div className="text-xs text-[var(--text-muted)] mb-1">{language === 'ar' ? 'إجمالي المدفوعات' : 'Total Payments'}</div>
                                            <div className="text-2xl font-bold text-amber-700">{profile?.payments?.length || 0}</div>
                                        </div>
                                        <div className="p-4 rounded-[var(--radius-md)] bg-gradient-to-br from-green-50 to-green-50/50 border border-green-200">
                                            <div className="text-xs text-[var(--text-muted)] mb-1">{language === 'ar' ? 'إجمالي السحوبات' : 'Total Payouts'}</div>
                                            <div className="text-2xl font-bold text-primary">{profile?.payouts?.length || 0}</div>
                                        </div>
                                    </div>

                                    {(profile?.wallet_transactions?.length || 0) > 0 ? (
                                        <div>
                                            <h4 className="text-sm font-bold text-[var(--text-muted)] mb-2">{language === 'ar' ? 'معاملات المحفظة' : 'Wallet Transactions'}</h4>
                                            <div className="overflow-x-auto rounded-[var(--radius-md)] border border-[var(--border)]">
                                                <table className="w-full text-sm">
                                                    <thead className="bg-[var(--light-bg)] text-xs text-[var(--text-muted)] uppercase">
                                                        <tr>
                                                    ...
                                                    <tbody className="divide-y divide-[var(--border)]">
                                                        {(profile?.wallet_transactions || []).map((t: any) => (
                                                            <tr key={t.id} className="hover:bg-[var(--light-bg)]">
                                                                <td className="px-4 py-2.5">
                                                                    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-bold ${t.type === 'credit' ? 'bg-primary-pale text-primary' : 'bg-red-100 text-red-700'}`}>
                                                                ...
                                                                <td className="px-4 py-2.5 text-xs text-[var(--text-muted)] max-w-[200px] truncate">{t.description || '-'}</td>
                                                                <td className="px-4 py-2.5 text-xs text-[var(--text-muted)]">{formatDateTime(t.created_at)}</td>
                                                            </tr>
                                                        ))}
                                                    </tbody>
                                                </table>
                                        ...
                                        <p className="text-center text-[var(--text-muted)] py-4">{language === 'ar' ? 'لا توجد معاملات' : 'No transactions'}</p>
                                    )}

                                    {(profile?.payouts?.length || 0) > 0 && (
                                        <div>
                                            <h4 className="text-sm font-bold text-[var(--text-muted)] mb-2">{language === 'ar' ? 'السحوبات' : 'Payouts'}</h4>
                                            <div className="overflow-x-auto rounded-[var(--radius-md)] border border-[var(--border)]">
                                                <table className="w-full text-sm">
                                                    <thead className="bg-[var(--light-bg)] text-xs text-[var(--text-muted)] uppercase">
                                                        <tr>
                                                    ...
                                                    <tbody className="divide-y divide-[var(--border)]">
                                                        {(profile?.payouts || []).map((p: any) => (
                                                            <tr key={p.id} className="hover:bg-[var(--light-bg)]">
                                                                ...
                                                                <td className="px-4 py-2.5">{getStatusBadge(p.status)}</td>
                                                                <td className="px-4 py-2.5 text-xs text-[var(--text-muted)]">{formatDate(p.requested_at || p.created_at)}</td>
                                                            </tr>
                                                        ))}
                                                    </tbody>
                                                </table>
                            ...
                            {activeTab === 'reviews' && (
                                <div className="space-y-4">
                                    {(profile?.reviews_given?.length || 0) > 0 && (
                                        <div>
                                            <h4 className="text-sm font-bold text-[var(--text-muted)] mb-2">{language === 'ar' ? 'التقييمات المُرسلة' : 'Reviews Given'}</h4>
                                            <div className="space-y-2">
                                                {(profile?.reviews_given || []).map((r: any) => (
                                                    <div key={r.id} className="p-3 rounded-[var(--radius-md)] border border-[var(--border)] flex items-start gap-3">
                                                        <div className="flex items-center gap-1 text-amber-500 flex-shrink-0">
                                                            <Star size={14} fill="currentColor" />
                                                            <span className="text-sm font-bold">{r.rating}</span>
                                                        </div>
                                                        <div className="flex-1 min-w-0">
                                                            <div className="text-sm text-navy">{r.comment || '-'}</div>
                                                            <div className="text-xs text-[var(--text-muted)] mt-1">
                                                                {language === 'ar' ? 'إلى' : 'To'}: {r.reviewedUser ? `${r.reviewedUser.first_name} ${r.reviewedUser.last_name}` : r.course?.name || '-'}
                                                            </div>
                                                        </div>
                                                        <div className="text-xs text-[var(--text-muted)] flex-shrink-0">{formatDate(r.created_at)}</div>
                                                    </div>
                                                ))}
                                            </div>
                                        </div>
                                    ...
                                    {(profile?.reviews_received?.length || 0) > 0 && (
                                        <div>
                                            <h4 className="text-sm font-bold text-[var(--text-muted)] mb-2 mt-4">{language === 'ar' ? 'التقييمات المستلمة' : 'Reviews Received'}</h4>
                                            <div className="space-y-2">
                                    ...
                                    {(!profile?.reviews_given?.length && !profile?.reviews_received?.length) && (
                                        <p className="text-center text-[var(--text-muted)] py-8">{language === 'ar' ? 'لا توجد تقييمات' : 'No reviews found'}</p>
                                    )}
                                </div>
                            )}
                                    ...
                                    {(profile?.support_tickets?.length || 0) > 0 ? (
                                        <div className="space-y-2">
                                            {(profile?.support_tickets || []).map((t: any) => (
                                                <div key={t.id} className="p-4 rounded-[var(--radius-md)] border border-[var(--border)] hover:border-primary/30 transition-all">
                                                    <div className="flex items-center justify-between mb-1">
                                                        <span className="font-semibold text-[var(--text-main)] text-sm">{t.subject}</span>
                                                        {getStatusBadge(t.status)}
                                                    </div>
                                                    <p className="text-xs text-[var(--text-muted)] line-clamp-2">{t.body}</p>
                                                    <div className="text-[10px] text-[var(--text-muted)] mt-1">{formatDateTime(t.created_at)}</div>
                                                </div>
                                            ))}
                                        </div>
                                    ) : (
                                        <p className="text-center text-[var(--text-muted)] py-8">{language === 'ar' ? 'لا توجد تذاكر دعم' : 'No support tickets found'}</p>
                                    )}
                                </div>
                            )}
                            ...
                            {activeTab === 'disputes' && (
                                <div>
                                    {(profile?.disputes?.length || 0) > 0 ? (
                                            ...
                                            <table className="w-full text-sm">
                                                ...
                                                <tbody className="divide-y divide-[var(--border)]">
                                                    {(profile?.disputes || []).map((d: any) => (
                                                        <tr key={d.id} className="hover:bg-[var(--light-bg)]">
                                                            <td className="px-4 py-2.5 text-sm">{d.reason}</td>
                                                            <td className="px-4 py-2.5">{getStatusBadge(d.status)}</td>
                                                            <td className="px-4 py-2.5 text-xs text-[var(--text-muted)]">{d.booking?.reference || '-'}</td>
                                                            <td className="px-4 py-2.5 text-xs text-[var(--text-muted)]">{formatDate(d.created_at)}</td>
                                                        </tr>
                                                    ))}
                                                </tbody>
                                            </table>
                                        ...
                                        <p className="text-center text-[var(--text-muted)] py-8">{language === 'ar' ? 'لا توجد نزاعات' : 'No disputes found'}</p>
                                    )}
                                </div>
                            )}
                        </div>
