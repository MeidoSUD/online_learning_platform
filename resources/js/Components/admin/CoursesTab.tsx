    ...
    return (
        <div className="space-y-6 animate-fade-in" onClick={() => setOpenMenuId(null)}>
            <div className="flex justify-between items-center">
                <h2 className="text-2xl font-bold text-[var(--text-main)]">{t.manageCourses}</h2>
            </div>
                ...
                <div className="overflow-x-auto min-h-[400px]">
                    <table className="w-full text-left text-sm">
                        <thead className="bg-[var(--light-bg)] border-b border-[var(--border)]">
                            <tr>
                                <th className="px-6 py-3 font-semibold text-navy">{t.course}</th>
                                <th className="px-6 py-3 font-semibold text-navy">{t.teacher}</th>
                                <th className="px-6 py-3 font-semibold text-navy">{t.category}</th>
                                <th className="px-6 py-3 font-semibold text-navy">{t.priceRange.split(' ')[0]}</th>
                                <th className="px-6 py-3 font-semibold text-navy">{t.approve}</th>
                        ...
                        <tbody className="divide-y divide-[var(--border)]">
                            {filteredCourses.map(course => (
                                <tr key={course.id} className="hover:bg-[var(--light-bg)] cursor-pointer" onClick={() => setSelectedCourse(course)}>
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            <div className="h-10 w-10 rounded-lg bg-[var(--light-bg)] overflow-hidden flex-shrink-0">
                                                {course.image ? (
                                            ...
                                            <div>
                                                <div className="font-bold text-[var(--text-main)] line-clamp-1">{course.name}</div>
                                                {course.is_featured ? (
                                                    ...
                                                    <div className="flex items-center gap-1 text-[10px] text-[var(--text-muted)] font-bold uppercase">
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase ${course.approval_status === 'approved' ? 'bg-primary-pale text-primary' :
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`h-2 w-2 rounded-full inline-block mr-2 ${normalizeStatus(course.status) === 'published' ? 'bg-primary' : 'bg-[var(--border)]'}`}></span>
                                    ...
                                    <td className="px-6 py-4 text-right relative">
                                        ...
                                        {openMenuId === course.id && (
                                            <div className={`absolute z-20 w-48 bg-white rounded-[var(--radius-md)] shadow-[var(--shadow-lg)] ring-1 ring-black ring-opacity-5 py-1 ${direction === 'rtl' ? 'left-8' : 'right-8'} top-8`}>
                                                <button onClick={(e) => { e.stopPropagation(); setSelectedCourse(course); }} className="w-full text-left px-4 py-2 text-sm hover:bg-[var(--light-bg)] flex items-center gap-2 text-navy">
                                                    <Eye size={16} /> {t.viewDetails}
                                                </button>

                                                {course.approval_status !== 'approved' && (
                                                    <button onClick={(e) => { e.stopPropagation(); handleApprove(course.id); }} className="w-full text-left px-4 py-2 text-sm hover:bg-[var(--light-bg)] flex items-center gap-2 text-primary">
                                                        <CheckCircle size={16} /> {t.approve}
                                                    </button>
                                                )}

                                                {course.approval_status !== 'rejected' && (
                                                    <button onClick={(e) => { e.stopPropagation(); setShowRejectionModal(true); setRejectingCourseId(course.id); }} className="w-full text-left px-4 py-2 text-sm hover:bg-[var(--light-bg)] flex items-center gap-2 text-amber-600">
                                                        <XCircle size={16} /> {t.reject}
                                                    </button>
                                                )}

                                                <button onClick={(e) => { e.stopPropagation(); handleToggleStatus(course.id); }} className="w-full text-left px-4 py-2 text-sm hover:bg-[var(--light-bg)] flex items-center gap-2 text-secondary">
                                                    <Filter size={16} /> {normalizeStatus(course.status) === 'published' ? t.draft : t.published}
                                                </button>

                                                <button onClick={(e) => { e.stopPropagation(); handleToggleFeatured(course.id, course.is_featured); }} className="w-full text-left px-4 py-2 text-sm hover:bg-[var(--light-bg)] flex items-center gap-2 text-amber-500">
                                                    <Star size={16} /> {course.is_featured ? t.removeFeatured : t.markFeatured}
                                                </button>

                                                <div className="border-t border-[var(--border)] my-1"></div>
                                                <button onClick={(e) => { e.stopPropagation(); handleDelete(course.id); }} className="w-full text-left px-4 py-2 text-sm hover:bg-[var(--light-bg)] flex items-center gap-2 text-red-600">
            ...
            <Modal isOpen={showRejectionModal} onClose={() => setShowRejectionModal(false)} title={t.rejectCourse}>
                <div className="space-y-4">
                    <p className="text-sm text-[var(--text-muted)]">{t.rejectionReasonLabel}</p>
                    <textarea
                        value={rejectionReason}
                        onChange={(e) => setRejectionReason(e.target.value)}
                        placeholder={t.phRejectionReason}
                        className="w-full h-32 p-3 rounded-lg border border-[var(--border)] focus:outline-none focus:border-primary resize-none"
                    />
            ...
            <Modal isOpen={!!selectedCourse} onClose={() => setSelectedCourse(null)} title={t.details}>
                {selectedCourse && (
                    <div className="space-y-6">
                        <div className="aspect-video w-full rounded-[var(--radius-md)] bg-[var(--light-bg)] overflow-hidden">
                        ...
                        <div>
                            <h3 className="text-xl font-bold text-[var(--text-main)] mb-2">{selectedCourse.title}</h3>
                            <p className="text-sm text-[var(--text-muted)] leading-relaxed">{selectedCourse.description}</p>
                        </div>

                        <div className="grid grid-cols-2 gap-4 text-sm">
                            <div className="p-3 bg-[var(--light-bg)] rounded-[var(--radius-md)]">
                                <span className="block text-xs text-[var(--text-muted)] mb-1">{t.teacher}</span>
                            ...
                            <div className="p-3 bg-[var(--light-bg)] rounded-[var(--radius-md)]">
                                <span className="block text-xs text-[var(--text-muted)] mb-1">{t.priceRange.split(' ')[0]}</span>
                                <span className="font-bold text-primary">{selectedCourse.price} {t.sar}</span>
                            </div>
                            <div className="p-3 bg-[var(--light-bg)] rounded-[var(--radius-md)]">
                                <span className="block text-xs text-[var(--text-muted)] mb-1">{t.category}</span>
                                <span className="font-bold">{selectedCourse.category?.name || t.na}</span>
                            </div>
                            <div className="p-3 bg-[var(--light-bg)] rounded-[var(--radius-md)]">
                                <span className="block text-xs text-[var(--text-muted)] mb-1">{t.approve}</span>
                                <span className={`font-bold capitalize ${selectedCourse.approval_status === 'approved' ? 'text-primary' :
                            ...
                            <div className="p-3 bg-[var(--light-bg)] rounded-[var(--radius-md)]">
                                <span className="block text-xs text-[var(--text-muted)] mb-1">{t.featured}</span>
                                <span className={`font-bold flex items-center gap-1 ${selectedCourse.is_featured ? 'text-amber-600' : 'text-[var(--text-muted)]'}`}>
                        ...
                        <div className="flex gap-2 pt-2">
                            {selectedCourse.approval_status !== 'approved' && (
                                <Button className="flex-1 bg-primary hover:bg-primary" onClick={() => handleApprove(selectedCourse.id)}>
                                    {t.approve} {t.course}
                                </Button>
                            )}
