...
import { Award, Search, Loader2, Plus, CheckCircle, XCircle, Eye, Trash2 } from 'lucide-react';
...
export const CertificatesTab: React.FC = () => {
  ...
  const fetchCertificates = async () => {
    setLoading(true);
    try {
      const data = await adminService.getCertificates({ search: searchTerm || undefined });
  ...
  const handleIssue = async () => {
    ...
    try {
      await adminService.issueCertificate({
        student_id: selectedEligible.user_id,
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">{language === 'ar' ? 'إصدار وإدارة شهادات التعلم' : 'Issue and manage learning certificates'}</p>
          </div>
        </div>
        <div className="flex gap-2">
          ...
          >
            {language === 'ar' ? 'ال صادرة' : 'Issued'}
          </button>
          <button
            onClick={() => setActiveView('eligible')}
      ...
      {loading ? (
        ...
        <>
              ...
              <table className="w-full text-sm">
                <thead className="bg-slate-50 border-b border-slate-200">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-start font-semibold text-slate-600">{language === 'ar' ? 'الدورة' : 'Course'}</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {paged.length === 0 ? (
                    <tr><td colSpan={5} className="px-4 py-12 text-center text-slate-400">{language === 'ar' ? 'لا توجد شهادات' : 'No certificates yet'}</td></tr>
                  ) : paged.map((cert) => (
          ...
          <Pagination currentPage={currentPage} totalPages={Math.ceil(certificates.length / ITEMS_PER_PAGE)} onPageChange={setCurrentPage} />
        </>
      ) : (
        <>
          <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-slate-50 border-b border-slate-200">
                  <tr>
                    <th className="px-4 py-3 text-start font-semibold text-slate-600">{language === 'ar' ? 'الطالب' : 'Student'}</th>
                    <th className="px-4 py-3 text-start font-semibold text-slate-600">{language === 'ar' ? 'هوية' : 'National ID'}</th>
                    <th className="px-4 py-3 text-start font-semibold text-slate-600">{language === 'ar' ? 'الدورة' : 'Course'}</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {eligible.length === 0 ? (
                    <tr><td colSpan={5} className="px-4 py-12 text-center text-slate-400">{language === 'ar' ? 'لا يوجد طلاب مؤهلين' : 'No eligible students'}</td></tr>
                  ) : eligible.map((s, i) => (
          ...
          <Pagination currentPage={currentPage} totalPages={Math.ceil(eligible.length / ITEMS_PER_PAGE)} onPageChange={setCurrentPage} />
        </>
      )}
        ...
        {selectedEligible && (
          <div className="space-y-4">
            <div className="bg-slate-50 rounded-xl p-4 space-y-2">
              ...
              <p className="text-sm"><span className="font-semibold text-slate-600">{language === 'ar' ? 'الدورة' : 'Course'}:</span> {selectedEligible.course_name}</p>
        ...
        {detailLoading ? (
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="bg-slate-50 rounded-xl p-3">
                <p className="text-xs font-semibold text-slate-400 uppercase">{language === 'ar' ? 'الدورة' : 'Course'}</p>
