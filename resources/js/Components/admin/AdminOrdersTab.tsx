...
export const AdminOrdersTab: React.FC = () => {
    ...
    const getStatusStyle = (status: string) => {
        switch (status) {
            ...
            case 'confirmed': return 'bg-secondary-pale text-secondary';
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            <div className="flex justify-between items-center">
                <h2 className="text-2xl font-bold text-[var(--text-main)]">{t.ordersManagement}</h2>
            </div>
                ...
                <div className="overflow-x-auto min-h-[400px]">
                    <table className="w-full text-left text-sm">
                        <thead className="bg-[var(--light-bg)] border-b border-[var(--border)]">
                            <tr>
                                <th className="px-6 py-3 font-semibold text-navy">Order ID</th>
                                <th className="px-6 py-3 font-semibold text-navy">{t.student}</th>
                                <th className="px-6 py-3 font-semibold text-navy">{t.subject}</th>
                                <th className="px-6 py-3 font-semibold text-navy">Apps</th>
                        ...
                        <tbody className="divide-y divide-[var(--border)]">
                            {filteredOrders.length === 0 ? (
                                ...
                                filteredOrders.map(order => (
                                    <tr key={order.id} className="hover:bg-[var(--light-bg)]">
                                        <td className="px-6 py-4 font-mono font-bold text-primary">
                                            #{order.id}
                                        </td>
                                        <td className="px-6 py-4">
                                            <div className="font-medium text-[var(--text-main)]">{order.student.first_name} {order.student.last_name}</div>
                                            <div className="text-xs text-[var(--text-muted)]">{order.student.phone_number}</div>
                                        </td>
                    ...
                    <div className="space-y-6">
                        <div className="grid grid-cols-2 gap-4">
                            <div className="p-4 bg-[var(--light-bg)] rounded-[var(--radius-md)] space-y-2">
                                <h4 className="text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider">{t.student} Info</h4>
                                <div className="flex items-center gap-3">
                                    <div className="h-10 w-10 rounded-full bg-primary/10 flex items-center justify-center text-primary">
                                        <User size={20} />
                                    </div>
                                    <div>
                                        <p className="font-bold text-[var(--text-main)]">{selectedOrder.student.first_name} {selectedOrder.student.last_name}</p>
                                        <p className="text-xs text-[var(--text-muted)]">{selectedOrder.student.email}</p>
                                    </div>
                                </div>
                            </div>
                            <div className="p-4 bg-[var(--light-bg)] rounded-[var(--radius-md)] space-y-2">
                                <h4 className="text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider">Service Request</h4>
                                <div className="flex items-center gap-3">
                                    <div className="h-10 w-10 rounded-full bg-secondary/10 flex items-center justify-center text-secondary">
                                        <Briefcase size={20} />
                                    </div>
                                    <div>
                                        <p className="font-bold text-[var(--text-main)]">{language === 'ar' ? selectedOrder.subject.name_ar : selectedOrder.subject.name_en}</p>
                                        <p className="text-xs text-[var(--text-muted)]">{selectedOrder.min_price} - {selectedOrder.max_price} SAR</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        {selectedOrder.notes && (
                            <div className="p-4 border border-[var(--border)] rounded-[var(--radius-md)]">
                                <h4 className="text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider mb-2">Student Notes</h4>
                                <p className="text-sm text-[var(--text-muted)] italic">"{selectedOrder.notes}"</p>
                            </div>
                        )}

                        <div className="space-y-4">
                            <div className="flex justify-between items-center">
                                <h4 className="font-bold text-[var(--text-main)]">Teacher Applications ({applications.length})</h4>
                                {appsLoading && <Loader2 className="animate-spin text-primary" size={16} />}
                            </div>

                            <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                                {applications.length === 0 ? (
                                    <div className="p-8 text-center bg-[var(--light-bg)] rounded-[var(--radius-md)] text-[var(--text-muted)] text-sm">
                                        No applications yet.
                                    </div>
                                ) : (
                                    applications.map(app => (
                                        <div key={app.id} className="p-4 border border-[var(--border)] rounded-[var(--radius-md)] hover:bg-[var(--light-bg)] transition-colors">
                                            <div className="flex items-center justify-between">
                                                <div className="flex items-center gap-3">
                                                    <div className="h-12 w-12 rounded-full bg-[var(--border)] overflow-hidden border border-[var(--border)]">
                                                        <img src={getStorageUrl(app.teacher.profile_photo)} alt="" className="h-full w-full object-cover" />
                                                    </div>
                                                    <div>
                                                        <div className="flex items-center gap-2">
                                                            <span className="font-bold text-[var(--text-main)]">{app.teacher.first_name} {app.teacher.last_name}</span>
                                                        ...
                                                        <p className="text-[10px] text-[var(--text-muted)]">{app.teacher.experience_years} years experience</p>
                                                    </div>
                                                </div>
                        ...
                        <div className="flex gap-2 pt-4 border-t border-[var(--border)]">
                            {selectedOrder.status === 'pending' && (
                                <button
                                    ...
                                    className="flex-1 py-2.5 px-4 bg-red-50 text-red-600 rounded-[var(--radius-md)] font-bold flex items-center justify-center gap-2 hover:bg-red-100 transition-all text-sm"
                                >
                            ...
                            {selectedOrder.status === 'confirmed' && (
                                <button
                                    ...
                                    className="flex-1 py-2.5 px-4 bg-indigo-50 text-indigo-600 rounded-[var(--radius-md)] font-bold flex items-center justify-center gap-2 hover:bg-indigo-100 transition-all text-sm"
                                >
                            ...
                            {selectedOrder.status === 'in_progress' && (
                                <button
                                    ...
                                    className="flex-1 py-2.5 px-4 bg-primary-pale text-primary rounded-[var(--radius-md)] font-bold flex items-center justify-center gap-2 hover:bg-primary-pale transition-all text-sm"
                                >
